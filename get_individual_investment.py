...
class GetIndividualInvestment:
    ...
    def _get_table(self):
        self.browser.wait_until_element_is_visible(
            locator=
            'css:div#investments-table-widget div.pageSelect select option:nth-of-type(4)',
            timeout=datetime.timedelta(minutes=1))
        self.browser.click_element(
            locator=
            'css:div#investments-table-widget div.pageSelect select option:last-of-type'
        )
        self.browser.wait_until_element_is_visible(
            locator=
            'css:div#investments-table-widget table#investments-table-object tbody tr:nth-of-type(11) td',
            timeout=datetime.timedelta(minutes=1))
    ...
    def _get_rows(self, table):
        self.browser.wait_until_element_is_enabled(
            locator=[table, 'css:table#investments-table-object tbody tr'])
    ...
    def parse(self):
        ...
        self.browser.wait_until_element_is_visible(
            locator=[
                table, 'css:table#investments-table-object tbody tr td a'
            ],
                    ...
                    if count_link > 0:
                        investment[
                            'link'] = self.browser.get_element_attribute(
                                locator=[cell, 'css:a'], attribute='href')
                    else:
                        investment['link'] = ''
